...
new client.Command("eval","Evaluates JS code","owner",async (msg,args)=>{
    ...
    try{
        ...
        repl = await msg.channel.send("",new Discord.RichEmbed({"title":"Return:","description":"```xl\n"+Discord.escapeMarkdown(util.inspect(ret).substring(0,2000),true)+"```","color":0x00ff00}))
    }catch(err){
        repl = await msg.channel.send("",new Discord.RichEmbed({"title":"Error: "+err.name,"description":err.message,"color":0xff0000}))
    }
    repl.react("❌")
